...
# ============================================================
# ---------------- ELLIPTICAL META-ATOM ---------------------
# ============================================================

def get_epgrid_ellipse(rx, ry, eps, a):
...
def reflection_coeff(lam, a, rx, ry, h=0.4, hsi=0.3):
    f = 1 / lam
    ...
    epgrid = get_epgrid_ellipse(rx, ry, eps_si, a)
...
lambdas = np.linspace(1.4, 1.8, 36)
a_vals  = np.linspace(0.8, 1.4, 36)
...
print("Running elliptical meta-atom λ–a sweep...\n")

for i, a in enumerate(a_vals):
    for j, lam in enumerate(lambdas):
        try:
            r00 = reflection_coeff(lam, a, rx, ry)
...
plt.title("Phase map: λ vs lattice constant a (ellipse)")
plt.tight_layout()
plt.show()
...
plt.xlabel("Wavelength (µm)")
plt.ylabel("Lattice constant a (µm)")
plt.title("Reflectance map: λ vs lattice constant a (ellipse)")
plt.tight_layout()
plt.show()
