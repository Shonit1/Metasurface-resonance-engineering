...
'''
this file hunts for slope that can be used to identify possible resonance by maximizing A but keeping the linear slope.
'''
...
# =========================================================
# BOUND HANDLER (CMA TRICK)
# =========================================================
def decode(x):
    """Map unconstrained CMA vector → physical bounds"""
    x = np.clip(x, 0.0, 1.0)
    return bounds[:, 0] + x * (bounds[:, 1] - bounds[:, 0])


# =========================================================
# LOSS FUNCTION (CMA SPACE)
# =========================================================
def loss_function_cma(x, alpha=0):
    ...
    print(f"A={A:.3f}, RMS={rms:.3e}, LOSS={loss:.3f}")
...
# CMA INITIALIZATION
# =========================================================
x0_phys = np.array([0.1, 0.120, 0.300, 0.300])
...
while not es.stop():
    solutions = es.ask()
    losses = [loss_function_cma(x) for x in solutions]
...
plt.title("CMA-optimized linear phase")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
