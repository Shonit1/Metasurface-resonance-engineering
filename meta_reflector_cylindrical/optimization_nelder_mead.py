...
lambdas = np.linspace(1.2, 2.1, 25)  # µm
...
def objective(params):
    ...
    # lattice-safe radius limit
    r_max_lattice = 0.5 * min(L1[0], L2[1])

    # hard constraints
    if not (0.05 <= r <= min(0.5, r_max_lattice)):
        return 1e3
    if not (0.005 <= h <= 1.2):
        return 1e3
    ...
    A, B = np.polyfit(x, phis, 1)
    phis_fit = A * x + B

    return np.sqrt(np.mean((phis - phis_fit)**2))



r_vals = np.linspace(0.05, min(0.45, 0.5*min(L1[0], L2[1])), 8)
h_vals = np.linspace(0.01, 0.9, 8)
hsio2_fixed = 0.3
...
plt.plot(lambdas, phis_fit, '--', label=r"Fit: $A\sqrt{\lambda}+B$")
