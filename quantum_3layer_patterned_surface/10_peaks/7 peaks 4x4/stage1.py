...
def solver_T(lam, pattern, hs_list, hsp_list, hs_dbr, a):
    try:
        ...
        ep = ep = get_epgrid_4x4(pattern, eps_si, a).flatten()
        obj.GridLayer_geteps(ep)
...
def loss_function(x):
    ...
    pattern = p[:16]
    hs_list = p[16:16+N_CAV]
    hsp_list = p[16+N_CAV:16+2*N_CAV]
    hs_dbr  = p[16+2*N_CAV]
    a       = p[16+2*N_CAV+1]
...
for i,(loss,p) in enumerate(top):
    ...
    for lam in lams:
        ...
        R,T = solver_T(lam,pattern,hs_list,hsp_list,hs_dbr,a)
        R.append(R)
        T.append(T)
